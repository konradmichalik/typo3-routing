...
- **Parameters & request bodies** — from the typed method signature: path/query parameters for `GET`-style routes, a JSON request body for `POST`/`PUT`/`PATCH`. Scalar types map to JSON Schema, backed enums become `enum` schemas, a `requirements` regex becomes a `pattern`, and a [`#[Param(description:)]`](USAGE.md#documenting-the-parameter) becomes the parameter's (or body property's) `description`.
