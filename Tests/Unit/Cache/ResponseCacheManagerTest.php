...
final class ResponseCacheManagerTest extends TestCase
{
    ...
    public function withCacheStatusAddsTheHeaderForACacheableGetRoute(): void
    {
        ...
        $response = $this->subject->withCacheStatus(new Response('php://temp', 200), $cacheConfig, Requests::get('https://example.com/api/x')->withoutNormalizedParams()->build(), 'HIT');
    ...
    {
        $response = $this->subject->withCacheStatus(new Response('php://temp', 200), null, Requests::get('https://example.com/api/x')->withoutNormalizedParams()->build(), 'MISS');
    ...
    {
        $cacheConfig = ['lifetime' => 3600, 'tags' => [], 'ignoreParams' => []];
        $response = $this->subject->withCacheStatus(new Response('php://temp', 200), $cacheConfig, Requests::post('https://example.com/api/x')->withoutNormalizedParams()->build(), 'MISS');
    ...
    public function buildKeyVariesByHost(): void
    {
        ...
        $siteA = $this->subject->buildKey('r', Requests::get('https://a.example.com/api/x')->withoutNormalizedParams()->build(), []);
        $siteB = $this->subject->buildKey('r', Requests::get('https://b.example.com/api/x')->withoutNormalizedParams()->build(), []);
    ...
    {
        return Requests::get('https://example.com/api/x')->withQueryParams($query)->withoutNormalizedParams()->build();
    }
}
