...
final readonly class Route
{
     ...
     * @param list<string>          $schemes         Allowed URI schemes (e.g. ['https']); empty = any scheme. A request that matched everything but the scheme answers `308 Permanent Redirect` to the same URL under the FIRST declared scheme (path, query, site base and port unchanged; `308` preserves method and body). Not gated behind `canonical` — declaring `schemes` already says which scheme the route answers on. A route invisible in the current context (env, sites, languages) still yields a 404, so the redirect can never reveal it. Not inherited from a class-level #[Route] (same rule as `methods`).
