...
| [Performance](performance.md) | Measured dispatch overhead, what the path gate costs unrelated traffic, and how to reproduce both |
...
The short version of all four: routes are compiled into the DI container once, a matched request costs about 0.6 ms more than a hand-written middleware, an unrelated page request pays under a microsecond, and `RouteRegistry` is a supported API that satellite packages can build on.
